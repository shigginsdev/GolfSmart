// Updated GolfScoreInput.js with automatic upload + scan
...
const GolfScoreInput = ({ user }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'courseName') {
      setFormData(prev => ({ ...prev, courseName: value, courseID: '' }));
    } else {
  ...
  const scanScorecard = async (imageUrl) => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        ...Object.entries(parsedScores).reduce((acc, [key, val]) => {
          acc[`Hole${key}Score`] = val.toString();
          return acc;
        }, {})
      }));
    } catch (err) {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      await scanScorecard(uploadedImageUrl);
    } catch (err) {
  ...
  const handleCourseSelect = (course) => {
    ...
    setFormData(prev => ({ ...prev, courseName, courseID: course.courseID }));
